 ...
 *{@code where: a_1, a_2 ... a_n are real numbers and b_1<b_2..<b_n >=0 are none negative integers (naturals)}
 ...
 * 3.5 double root(double x0, double x1, double eps) //{@code assuming (f(x0)*f(x1)<=0, returns f(x2) such that:}
 *													//{@code	(i) x0<=x2<=x2 & (ii) f(x2)<eps}
...
public interface Polynom_able extends cont_function{
	 ...
	 * @return true if zero polynom
	 */
	public boolean isZero();
	/**
	 * {@code Compute a value x' (x0<=x'<=x1) for with |f(x')| < eps}
	 * {@code assuming (f(x0)*f(x1)<=0, returns f(x2) such that:}
	 * *	{@code(i) x0<=x2<=x2 && (ii) f(x2)<eps }
	 * @param x0 starting point
	 * @param x1 end point
	 * @param eps step (positive) value
	 * @return root of polynom
	 */
